...
for(j in c(1:17, 19)){
  ...
  for(index.test in index.list){
    ...
    model <- gbm(mal~., data=train, verbose = F)
    ...
    for(i in 1:100){
      ...
      pred.prob <- predict(object = model, newdata = test, type = 'response')
      result.feature <- cbind(result.feature, pred.prob)
    }
    result.feature <- cbind(result.feature, index.test)
    result <- rbind(result, result.feature)
  }
  rownames(result) <- result$index.test
...
saveRDS(result.importance, 'importance_gbm.rds')
