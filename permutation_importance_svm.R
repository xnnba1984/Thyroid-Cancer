...
for(j in c(1:17, 19)){
  ...
  for(index.test in index.list){
    ...
    model <- svm(mal~., kernel='radial', data=train, probability=T)
    ...
    for(i in 1:100){
      ...
      pred.prob <- attr(predict(model, test, probability=T),'probabilities')[,1]
...
saveRDS(result.importance, 'importance_svm_radial.rds')
