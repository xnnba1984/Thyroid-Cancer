...
system.time({
  ...
  for(i in 1:boost.count){
    ...
    for(index.test in index.list){
      ...
      result <- rbind(result, cbind(index.test, pred.prob))
    }
...
saveRDS(result.boostrap, "result/result_boostrap_logistic.rds")
#result.boostrap <- readRDS("result_boostrap_bagging.rds")
