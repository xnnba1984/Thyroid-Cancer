...
for(j in c(1:17, 19)){
  ...
  data <- data[,-1]
  cat(colnames(data)[j], '\n')
  index.list <- createFolds(1:dim(data)[1], k = 10, list = T, returnTrain = F)
  result <- data.frame()
  ...
  for(index.test in index.list){
    ...
    model <- glm(mal~., family = 'binomial', data = train)
...
saveRDS(result.importance, 'result/importance_logistic.rds')
